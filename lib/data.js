...
export const getJobsPosted = async (user_id, prisma) => {
  ...
  // job.map(async (job) => {
  //   return job.applications = await getJobApplications(job, prisma)
...
// Once the user applied to a job, we’ll show “already applied” and won’t let adding new applications from them to this specific job, this function return only true or false
